...
function App() {
  ...
  return (
    <section className="relative w-full min-h-screen sm:h-screen overflow-hidden">
      ...
      {/* Dimmed Overlay for better content contrast */}
      <div className="absolute inset-0 bg-white/20 sm:bg-transparent pointer-events-none z-[1]" />
      ...
      <nav className="absolute top-0 left-0 right-0 z-30 flex items-center justify-between px-4 sm:px-6 md:px-10 py-4 sm:py-6">
        ...
        <div className="hidden lg:flex items-center gap-1 bg-white/75 backdrop-blur-md rounded-full pl-6 pr-1 py-1 shadow-sm border border-white/60">
          {navLinks.map((link, i) => (
            <a
              key={link.href}
              href={link.href}
        ...
        <div className="flex items-center gap-3 sm:gap-6 text-[#2d3a2a]">
          <a href="#signup" className="hidden sm:flex items-center gap-2 text-sm font-medium hover:opacity-85 transition-opacity">
            <UserPlus className="w-4 h-4" />
            Sign Me Up!
          </a>
          <a href="#login" className="hidden sm:flex items-center gap-2 text-sm font-medium hover:opacity-85 transition-opacity">
      ...
      <div className="relative z-10 flex flex-col items-center text-center pt-24 sm:pt-28 md:pt-32 px-4 sm:px-6">
        ...
        <p className="mt-6 sm:mt-8 text-[#4b5b47] text-sm sm:text-base md:text-lg leading-relaxed max-w-md px-2 font-medium">
      ...
      <div className="absolute left-4 right-4 sm:right-auto sm:left-6 md:left-10 bottom-6 sm:bottom-8 md:bottom-10 z-10 max-w-sm">
        ...
        <p className="text-[#3d5638]/90 sm:text-white/85 text-xs leading-relaxed mb-6 max-w-xs font-semibold sm:font-normal">
      ...
      <div className="hidden sm:flex absolute right-6 md:right-10 bottom-8 md:bottom-10 z-10 items-center gap-2 text-white/90 text-sm bg-black/15 backdrop-blur-xs px-3 py-1.5 rounded-full">
        <button className="flex items-center justify-center w-6 h-6 rounded-full bg-white/25 hover:bg-white/40 transition-colors cursor-pointer">
          <Play className="w-3 h-3 fill-white text-white ml-0.5" />
        </button>
        <span className="font-medium">How we build?</span>
        <span className="text-white/70">1:35</span>
      </div>
    </section>
  );
}
